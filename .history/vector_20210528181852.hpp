...
ft::Vector<T, Alloc>(const allocator_type &alloc = allocator_type()) : _capacity(NULL), _size(NULL), _data(NULL), _alloc(alloc)
{}

template<class T, class Alloc>
ft::Vector<T, Alloc>(size_type n, const value_type & val = value_type(), const allocator_type& alloc = allocator_type())
...
ft::Vector<T, Alloc>::Vector(const ft::Vector<T, Alloc> & x)
{
	...
	_data = _alloc.allocate(vector._capacity);
	_size = vector._size;

	for (size_type i = 0; i < _size; i++)
		_alloc.construct(_data + i, vector[i]);
}
...
template <class InputIterator>
ft::Vector<T, Alloc>(InputIterator first, InputIterator last, const allocator_type &alloc = allocator_type());
