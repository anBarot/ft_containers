...
template<class T, class Alloc>
ft::Vector<T, Alloc>::Vector(size_type n_size, const_reference init) : _capacity(n_size), _size(n_size)
{
	_ptr = new T[n_size];
...
template<class T, class Alloc>
ft::Vector<T, Alloc>::Vector(std::initializer_list<T> l) : _size(l.size()), _capacity(l.size())
{
	_ptr = new T[_size];
	int i = 0;
...
template<class T, class Alloc>
ft::Vector<T, Alloc> &ft::Vector<T, Alloc>::operator=(const Vector<T> &vector)
{
	this->_size = vector._size;
	this->_capacity = vector._capacity;
	this->_ptr = new T[vector._size];
...
class ft::Vector<T, Alloc>::Iterator
{
	...
	public:
	    ...
	    Iterator(const Vector<T, Alloc> *vector, size_type index) : _pvector(vector), _index(index)	{}
