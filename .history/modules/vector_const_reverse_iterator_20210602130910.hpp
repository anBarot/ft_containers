...
class ft::Vector<T, Alloc>::Const_Reverse_Iterator
{
	...
	public:
	    ...
	    const_reverse_iterator operator--()				{ ++this->_index; return (*this); }
		const_reverse_iterator operator++(int)			{ iterator tmp(*this); --this->_index; return(tmp); }
    	const_reverse_iterator operator--(int)			{ iterator tmp(*this); ++this->_index; return(tmp); }
		const_reverse_iterator operator+(size_type i)		{ iterator tmp(this->_pvector, this->_index - i); return(tmp); }
    	const_reverse_iterator operator-(size_type i)		{ iterator tmp(this->_pvector, this->_index + i); return(tmp); }
