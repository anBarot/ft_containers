...
namespace ft
{
	...
	class Vector
	{
		...
		public:
			...
			:  Vector(first, last, alloc, typename std::iterator_traits<InputIterator>::iterator_category()) {}
			template <class InputIterator>
  			Vector(InputIterator first, InputIterator last, const allocator_type &alloc = allocator_type(), std::input_iterator_tag)
			: _alloc(alloc),_data,_capacity,_size; 
			{
				while (first != last)
