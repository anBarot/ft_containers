...
namespace ft
{
	...
	class Vector
	{
		...
		public:
			// Coplien
			explicit Vector(const allocator_type &alloc = allocator_type()) : _capacity(NULL), _size(NULL), _ptr(NULL), _alloc(alloc)	{}
			...
			~Vector()	{
							this->clear(); 
							_alloc.deallocate(this->_ptr, this->_size);
						};
...
template<class T>
ft::Vector<T>::Vector(const ft::Vector<T, Alloc>&vector)
{
