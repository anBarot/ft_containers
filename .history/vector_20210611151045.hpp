...
namespace ft
{
	...
	class Vector
	{
		...
		public:
			...
			// Iterator
			iterator				begin()			{ iterator tmp(_data); return (tmp); }
			const_iterator			begin() const 	{ const_iterator tmp(_data); return (tmp); }
			iterator				end() 			{ iterator tmp(_data + _size); return (tmp); }
			const_iterator			end() const		{ const_iterator tmp(_data + _size); return (tmp); }
